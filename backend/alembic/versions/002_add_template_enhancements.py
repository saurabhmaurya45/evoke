"""Add template enhancements: categories, currencies, and template fields.
...
def upgrade() -> None:
    ...
    op.add_column('templates', sa.Column('pricing_model', sa.String(20), nullable=True, server_default='FREE'))
    ...
    op.add_column('templates', sa.Column('storefront_status', sa.String(20), nullable=True, server_default='LISTED'))
    ...
    op.add_column('template_versions', sa.Column('capabilities', sa.JSON(), nullable=True, server_default='{}'))
    op.add_column('template_versions', sa.Column('defaults', sa.JSON(), nullable=True))

    # Rename default_config to defaults (migrate existing data)
    op.execute('UPDATE template_versions SET defaults = default_config WHERE defaults IS NULL')
    op.drop_column('template_versions', 'default_config')


def downgrade() -> None:
    # Restore default_config column
    op.add_column('template_versions', sa.Column('default_config', sa.JSON(), nullable=True))
    op.execute('UPDATE template_versions SET default_config = defaults WHERE default_config IS NULL')
    ...
    op.drop_index('idx_templates_storefront_status')
