...
@Component({
          ...
          @else {
            ...
            @if (mode() === 'login' || mode() === 'signup') {
              <form [formGroup]="form" (ngSubmit)="submit()" novalidate>
                @if (mode() === 'signup') { <div class="row"><label>First name<input formControlName="firstName" autocomplete="given-name" /></label><label>Last name<input formControlName="lastName" autocomplete="family-name" /></label></div> }
                <label>Email address<input type="email" formControlName="email" autocomplete="email" placeholder="you@example.com" /><small>{{ fieldError('email') }}</small></label>
                <label>Password<div class="password"><input [type]="showPassword() ? 'text' : 'password'" formControlName="password" autocomplete="current-password" /><button type="button" (click)="showPassword.set(!showPassword())" [attr.aria-label]="showPassword() ? 'Hide password' : 'Show password'">{{ showPassword() ? 'Hide' : 'Show' }}</button></div><small>{{ fieldError('password') }}</small></label>
                @if (mode() === 'signup') { <label>Confirm password<input type="password" formControlName="confirmPassword" autocomplete="new-password" /></label><label class="check"><input type="checkbox" formControlName="terms" /> I agree to the Terms and Privacy Policy</label> }
                @if (mode() === 'login') { <div class="form-meta"><label class="check"><input type="checkbox" formControlName="remember" /> Remember me</label><a routerLink="/forgot-password">Forgot password?</a></div> }
  ...
  styles: [`
    @use 'abstracts' as *;
    :host { display:block; height:100dvh; overflow:hidden; } .auth-shell { height:100dvh; display:grid; grid-template-columns:1fr 1fr; background:var(--bg); color:var(--text); overflow:hidden; }
...
export class AuthPageComponent {
  protected readonly auth = inject(AuthService); private readonly route = inject(ActivatedRoute); private readonly router = inject(Router);
